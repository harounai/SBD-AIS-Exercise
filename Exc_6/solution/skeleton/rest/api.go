...
func GetReceiptFile(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// handle any error!
		if err != nil {
			slog.Error("Unable to read receipt file from S3", slog.String("error", err.Error()))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, "Unable to read receipt file")
			return
		}
		// serve file
		...
		if err != nil {
			slog.Error("Unable to write receipt file to response", slog.String("error", err.Error()))
		}
		// todo
...
func PostOrder(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_, err = s3.PutObject(r.Context(), storage.OrdersBucket, dbOrder.GetFilename(), reader, int64(len(content)), minio.PutObjectOptions{ContentType: "text/markdown"})
		if err != nil {
